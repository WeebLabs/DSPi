...
void spdif_input_update_clock_servo(void) {
    ...
    // MCK is now generated by CLK_GPOUTn (hardware clock generator), not PIO,
    // so the divider math has no factor-of-2 toggle inflation:
    //   MCK divider (24.8) = sys_clk × 256 / (actual_freq × multiplier)
    // (Previous PIO route used × 128 because the toggle program produced one
    //  MCK cycle every 2 PIO cycles.)
