...
bool is_pin_in_use(uint8_t pin, uint8_t exclude) {
    ...
    // Reserve the I2S BCK/LRCLK pin pair unconditionally — they may be
    // owned by the I2S clock domain (TX master, phantom, or RX observation)
    // at any time. Letting another consumer claim them would cause silent
    // mis-sampling next time an I2S participant activates.
    if (pin == i2s_bck_pin || pin == (uint8_t)(i2s_bck_pin + 1u)) return true;
    ...
    // Check I2S DIN input pin
    if (pin == i2s_din_pin) return true;
    return false;
}
...
static void vendor_handle_set_data(tusb_control_request_t const *req) {
    ...
    switch (vendor_last_request) {
        ...
        case REQ_SET_INPUT_SOURCE: {
            ...
            // pipeline reset and hardware start/stop.
            // Notification is emitted at apply time (main.c), not here —
            // the shadow tracks *active* state, not requested state.
            //
            // Phase 2.5: any combination of I2S input + I2S outputs is now
            // supported. The library's election manages BCK/LRCLK ownership.
            if (buffer->data_len >= 1) {
        ...
        switch (setup->bRequest) {
            ...
            case REQ_SET_OUTPUT_TYPE: {
                ...
                if (slot >= NUM_SPDIF_INSTANCES) {
                    ...
                    // Phase 2.5: I2S TX may coexist with I2S input — the
                    // library's election re-elects on type changes and
                    // installs/uninstalls the phantom clkout-only SM as
                    // needed. No mutual-exclusion check required here.
            ...
            case REQ_SET_I2S_BCK_PIN: {
                ...
                if (!is_valid_gpio_pin(new_pin) || !is_valid_gpio_pin(new_pin + 1)) {
                    ...
                    // Reject if any I2S participant currently owns BCK/LRCLK:
                    // an I2S TX output (clkout master), or I2S input (phantom
                    // SM or RX self-syncing on these GPIOs).
                    ...
                    if (any_i2s) {
                        ...
                        i2s_bck_pin = new_pin;
                        // Refresh the library's clock-domain config so a
                        // future phantom install uses the updated pin.
                        audio_i2s_clock_domain_configure(
                            i2s_bck_pin,
            ...
            case REQ_SET_MCK_PIN: {
                // MCK is generated by CLK_GPOUTn (hardware clock generator),
                // so the chosen GPIO must map to a clk_gpout instance.
                // GPIO_TO_GPOUT_CLOCK_HANDLE returns clk_gpout0..3 for
                // gpout-capable pins or `default_clk_handle` (here, the
                // sentinel `clk_sys`) otherwise.  is_valid_gpio_pin() then
                // also rejects DSPi-reserved pins (12 UART, 23-25 power/LED).
                uint8_t new_pin = (uint8_t)setup->wValue;
                uint8_t status;
                if (!is_valid_gpio_pin(new_pin)) {
                    status = PIN_CONFIG_INVALID_PIN;
                } else if (GPIO_TO_GPOUT_CLOCK_HANDLE(new_pin, clk_sys) == clk_sys) {
                    status = PIN_CONFIG_INVALID_PIN;  // not gpout-capable
            ...
            case REQ_SET_I2S_DIN_PIN: {
                // Mirror REQ_SET_SPDIF_RX_PIN: refuse while the input owning
                // this pin is active, refuse pin collisions, otherwise update
                // and queue a directory flush.
                uint8_t new_pin = (uint8_t)setup->wValue;
                uint8_t status;
                if (!is_valid_gpio_pin(new_pin)) {
                    status = PIN_CONFIG_INVALID_PIN;
                    ...
                    flash_set_i2s_din_pin_pending = true;
                    status = PIN_CONFIG_SUCCESS;
            ...
            case REQ_GET_I2S_DIN_PIN: {
                resp_buf[0] = i2s_din_pin;
                vendor_send_response(resp_buf, 1);
                return true;
            }

            case REQ_GET_I2S_INPUT_STATUS: {
                I2sInputStatusPacket status;
                i2s_input_get_status(&status);
                vendor_send_response(&status, sizeof(status));
                return true;
            }
        }
