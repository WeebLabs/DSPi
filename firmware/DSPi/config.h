...
// Prefill thresholds: how full the consumer pool gets before outputs are
// enabled after an input source change.  Sets where the pool's natural
// ±4-buffer oscillation (one input batch = 192 samples = 4 buffers) is
// centred and therefore the steady-state output latency.
//
//   prefill=8 (50%): oscillates 4..8, avg 6 buffers ≈ 6 ms output latency,
//                    4 ms jitter margin before silence insertion (default
//                    for SPDIF — needed for clock-servo / FIFO-jitter slack)
//   prefill=6 (37%): oscillates 2..6, avg 4 buffers ≈ 4 ms output latency,
//                    2 ms jitter margin (recommended I2S Pico-master default)
//   prefill=4 (25%): oscillates 0..4, avg 2 buffers ≈ 2 ms output latency,
//                    0 ms jitter margin — ANY late batch causes a silence
//                    buffer (~1 ms audible click).  Aggressive; tune only
//                    after verifying clean behaviour on target hardware.
//
// I2S input is Pico-master so input/output clocks share sys_clk via
// identical PIO dividers — no clock drift, only main-loop jitter to absorb.
// SPDIF input has clock servo correction overlaying batch jitter, so it
// keeps the more conservative 50% threshold.
#define SPDIF_PREFILL_BUFFER_COUNT  (SPDIF_CONSUMER_BUFFER_COUNT / 2)
#define I2S_PREFILL_BUFFER_COUNT    6
...
#define REQ_SET_I2S_DIN_PIN         0xCA  // wValue = GPIO pin, returns status byte
#define REQ_GET_I2S_DIN_PIN         0xCB  // returns uint8_t
#define REQ_GET_I2S_INPUT_STATUS    0xCC  // returns 16-byte I2sInputStatusPacket
...
// Master clock is generated by CLK_GPOUTn (hardware clock output, not PIO).
// GPIO 21 = clk_gpout0 on both RP2040 and RP2350 — only DSPi-friendly
// gpout pin (23-25 are reserved for SMPS/VBUS/LED).  RP2350 also offers
// GPIO 13 / 15 as alternate gpout pins, but 21 is portable across both.
#define PICO_I2S_MCK_PIN            21   // MCK on CLK_GPOUT0 (optional)

// I2S clock-domain phantom SM placement (clkout-only, used when ≥1 I2S RX
// is registered but no I2S TX is registered). The phantom drives BCK/LRCLK
// on the configured clock_pin_base GPIO pair via PIO side-set.
#if PICO_RP2350
#define I2S_CLOCK_DOMAIN_PHANTOM_PIO_IDX  2u   // pio2/SM1 — pio2 hosts I2S RX SM0
#define I2S_CLOCK_DOMAIN_PHANTOM_SM       1u   // when input is I2S; SPDIF RX uses
                                                // pio2/SM0 only when input is SPDIF
                                                // (mutually exclusive at the input source layer)
#else
#define I2S_CLOCK_DOMAIN_PHANTOM_PIO_IDX  0u   // pio0/SM2 — free in all common configs
#define I2S_CLOCK_DOMAIN_PHANTOM_SM       2u   // (SPDIF outputs use pio0/SM0-1)
#endif
