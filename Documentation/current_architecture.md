...
| I2S RX channel | CH7 (write-side ring) | CH7 (write-side ring) |
| PDM channel | Dynamic (claim) | Dynamic (claim) |

### I2S Input PIO Layout (Pico-master, Phase 2.5)

| Feature | RP2040 | RP2350 |
|---------|--------|--------|
| Phantom (clkout-only) SM placement | pio0 / SM2 | pio2 / SM1 |
| Phantom installed only when | 0 TX + ≥1 RX | 0 TX + ≥1 RX |
| TX-master role (when ≥1 TX) | TX0 SM (combined `audio_i2s_clkout`) | TX0 SM (combined `audio_i2s_clkout`) |
| RX SM (DIN sampling) | pio1 / SM2 | pio2 / SM0 |
| Coexists with any I2S TX combination | Yes | Yes |
...
| I2S input ring (2048 × 4, 8 KB-aligned) | 8 KB |
| Other BSS | ~24 KB |
| **Total BSS** | **~218 KB** (was ~210 KB pre-I2S-input) |
...
| REQ_SET_I2S_DIN_PIN | 0xCA | IN* | Set I2S DIN GPIO pin (wValue=pin, returns status); rejected while I2S input active |
| REQ_GET_I2S_DIN_PIN | 0xCB | IN | Get I2S DIN GPIO pin |
| REQ_GET_I2S_INPUT_STATUS | 0xCC | IN | Get I2S input status (16-byte I2sInputStatusPacket) |
...
| REQ_SET_INPUT_SOURCE | 0xE0 | OUT | Set active input source (0=USB, 1=SPDIF, 2=I2S); rejects I2S if any output is I2S |
...
- **MCK:** Generated by the hardware **CLK_GPOUTn** clock generator, **not a PIO SM** — clk_sys is divided by the CLK_GPOUTn 24.8 divider and routed directly to the GPIO pad. Default GPIO 21 (= clk_gpout0 on both RP2040 and RP2350); RP2350 also accepts GPIO 13 / 15. No PIO SM is consumed for MCK. Lower jitter than the prior PIO toggle program, and integer dividers for 48 kHz × 256× (was fractional 12.5 with PIO) and 96 kHz × 128× (was fractional 12.5 with PIO).
...
- **MCK enable order:** the centralised `update_mck_for_i2s_state()` helper in `main.c` (called by `process_type_switches`, `perform_rate_change`, the boot path, the input-source switch, the bulk-params and preset-load handlers, and factory reset) writes the MCK clkdiv via `audio_i2s_mck_update_frequency()` *before* calling `audio_i2s_mck_set_enabled(true)`, so the SM starts at the correct frequency rather than briefly running at the previous divider. When MCK is already running, only the divider is updated. Matches the `REQ_SET_MCK_ENABLE` vendor command order.
...
| Signal | Source | Frequency | Divider | Jitter |
|--------|--------|-----------|---------|--------|
| I2S BCK (Fs×64) | PIO clkout (SM divider) | 3.072 MHz | 100.0 PIO clocks | Zero |
| MCK 128× | CLK_GPOUT0 (24.8 divider on clk_sys) | 6.144 MHz | 50.0 | Zero |
| MCK 256× | CLK_GPOUT0 (24.8 divider on clk_sys) | 12.288 MHz | 25.0 | Zero (was 12.5 / fractional with PIO route) |
| MCK 128× @ 96 kHz | CLK_GPOUT0 | 12.288 MHz | 25.0 | Zero (was 12.5 / fractional with PIO route) |
| MCK 256× @ 96 kHz | CLK_GPOUT0 | 24.576 MHz | 12.5 | Fractional (sanitiser forces 128× at 96 kHz) |
...
Abstraction layer enabling selection between multiple audio input sources. Currently supports USB (default), SPDIF, and I2S (Pico-master). Designed for future extensibility to ADAT and I2S-slave without restructuring.
...
- `WireInputConfig` (16 bytes) section in `WireBulkParams` V7+ — now also carries `i2s_din_pin` (consumed reserved byte, no `WIRE_FORMAT_VERSION` bump)
- SPDIF RX pin stored in `PresetDirectory` (consumed existing padding byte, no directory format change)
- I2S DIN pin stored in `PresetDirectory` v3 (added field; v2→v3 migration defaults to `PICO_I2S_DIN_PIN_DEFAULT`)

---

## I2S Input (Pico-master)
*Last updated: 2026-04-29*

Captures stereo I2S audio from an external source (typically an I2S ADC) where the Pico generates BCK/LRCLK and (optionally) MCK. Phase 2 supports stereo I2S only; the library-level abstraction is parameterised for future TDM and multi-DIN N-channel input. **Phase 2.5 lifted the Phase-2 mutual-exclusion limitation** — I2S input may coexist with any combination of I2S TX outputs.

### Files

- `i2s_input.h/c` — High-level subsystem (lifecycle, ring drain, preamp, source switching)
- `firmware/pico-extras/src/rp2_common/pico_audio_i2s_multi/audio_i2s_datain.pio` — RX PIO program
- `firmware/pico-extras/src/rp2_common/pico_audio_i2s_multi/audio_i2s_clkout_only.pio` — Clocks-only master PIO program (drives BCK/LRCLK without TX)

### Clocking

Pico is the I2S master: BCK/LRCLK and the RX SM all derive from `sys_clk` via identical 24.8 dividers (`sys_clk * 2 / Fs`). Input rate ≡ output rate by construction — no clock servo, no resampler, no fill trim. Sample rate is host-driven (`audio_state.freq`); rate changes propagate through `perform_rate_change()`, which now also calls `i2s_input_set_frequency()` and `audio_i2s_clock_domain_clkout_only_set_frequency()`.

### Library-managed clock-domain election (Phase 2.5)

The library `pico_audio_i2s_multi` runs an internal election on every TX/RX setup/teardown to decide which SM drives BCK/LRCLK. The election rule:

| Registered participants | Master SM | Program | Phantom |
|---|---|---|---|
| ≥1 TX (any RX) | `i2s_instances[0]` | `audio_i2s_clkout` (combined clocks+data) | No (fold-in optimization preserves SM economy) |
| 0 TX, ≥1 RX | configured phantom slot (RP2040 pio0/SM2; RP2350 pio2/SM1) | `audio_i2s_clkout_only` (clocks only) | Yes |
| 0 TX, 0 RX | none | — | No (cleared) |

Other TX instances run `audio_i2s_dataout` (data only). RX instances run `audio_i2s_datain` and self-sync via `wait gpio LRCLK` at program entry. The library exposes a transaction API (`audio_i2s_clock_domain_begin_transaction` / `_commit_transaction`) so multi-step orchestrators (e.g. `process_type_switches`) trigger a single re-election at commit instead of one per setup/teardown.

Boot wiring in `usb_audio.c:usb_sound_card_init`:
...
- `process_type_switches`: wraps teardown/setup loop in `audio_i2s_clock_domain_begin/commit_transaction`. The `clock_master` field in `audio_i2s_config_t` is now ignored (election decides roles internally).
- `i2s_input.c`: calls `audio_i2s_rx_setup`/`_teardown`. The election in those functions installs/uninstalls the phantom automatically.
- `vendor_commands.c`: `REQ_SET_I2S_BCK_PIN` rejects when any I2S participant (TX or RX) is active; on success it calls `audio_i2s_clock_domain_configure` to update the library.

Mutual-exclusion guards from Phase 2 are removed: any combination of I2S TX outputs and I2S input is supported.

### Hardware Layout

| Platform | Clocks-only master SM | RX SM      | DMA channel |
|----------|-----------------------|------------|-------------|
| RP2040   | pio0 / SM2            | pio1 / SM2 | 7           |
| RP2350   | pio2 / SM1            | pio2 / SM0 | 7           |

The RX SM uses absolute `wait gpio` on BCK/LRCLK (patched at runtime via `pio_encode_wait_gpio()`), so it does not need to share a PIO block with the clocks-only master.

### State Machine

`I2sInputState`: INACTIVE → STARTING → ACTIVE. Pico-master can't lose lock, so no RELOCKING state.

- `i2s_input_start()` brings up the clocks-only master, optionally enables MCK, sets up the RX instance, and enters STARTING with outputs muted via the existing `prepare_pipeline_reset` machinery.
- The main-loop polling block transitions STARTING → ACTIVE once consumer slot 0 fill reaches 50%, mirroring the SPDIF prefill discipline (`drain_and_disable_outputs` → wait → `enable_outputs_in_sync`).
- `i2s_input_stop()` tears down RX SM/DMA and stops the clocks-only master, releasing BCK/LRCLK to high-Z.

### DMA Ring

Single DMA channel writes to a caller-allocated 8 KB ring (`uint32_t i2s_ring[2048]` aligned to 8 KB) via the hardware write-side ring (`channel_config_set_ring(true, 13)`). The transfer count is set to `0xFFFFFFFF` — the channel runs effectively forever; the consumer derives the current write position from `dma_hw->ch[c].write_addr` and computes available samples modulo ring size.

### Audio Extraction

Mirrors SPDIF input format-handling (`spdif_input_poll`):
- Frame format: 32-bit per slot, 24-bit audio left-justified MSB-first.
- Per-stereo-pair: `raw_l = (int32_t)(word_l & 0xFFFFFF00u)` (sign-extends naturally).
- Per-channel preamp applied identically to USB and SPDIF: float on RP2350 via `global_preamp_linear[]`, Q28 on RP2040 via `fast_mul_q28()` and `global_preamp_mul[]`.
- Filled `buf_l[]`/`buf_r[]` are passed to `process_input_block()` — the canonical DSP entry.

**Batching:** `i2s_input_poll` holds off calling `process_input_block` until 192 stereo pairs have accumulated in the ring (or the ring exceeds 75% capacity, whichever comes first). Without this gate, the main loop polls fast enough that each call would process only 2-10 stereo pairs and the per-call fixed overhead in `process_input_block` (pool takes, mute envelope, watermark, DSP setup) would dominate — pinning the CPU load reading at ~70% even though the actual DSP work is small. 192 stereo pairs matches SPDIF's natural IEC 60958 block delivery, so input latency and CPU efficiency are identical across the two input sources. Overflow guard at 75% ring fill protects against starvation during flash blackouts. *Last updated: 2026-05-03*

**Prefill threshold (output side):** The I2S input path uses a separate `I2S_PREFILL_BUFFER_COUNT` (default 6 of 16 ≈ 37%) instead of SPDIF's `SPDIF_PREFILL_BUFFER_COUNT` (8 of 16 = 50%). Pico-master shares `sys_clk` with outputs via identical PIO dividers, so input rate is hardware-locked to output rate — the prefill only needs to absorb main-loop jitter, not clock drift. Saves ~2 ms of steady-state output latency vs the SPDIF threshold while keeping a 2-buffer (~2 ms) jitter margin. The pool naturally oscillates ±4 buffers (one input batch); see `config.h:I2S_PREFILL_BUFFER_COUNT` for the latency / jitter-margin trade-off table. *Last updated: 2026-05-03*

**PIO timing (audio_i2s_datain.pio):** The RX program is structurally cycle-identical to `audio_i2s_dataout.pio`: 31-iteration `in/jmp` loop + post-loop `in` + `set x, 30`, no `[delay]` modifiers anywhere. Steady-state per-frame cycle count from `.wrap_target` to `.wrap` is exactly 128 — matching the 128-cycle clock master. The `wait gpio LRCLK` / `wait gpio BCK` instructions are placed BEFORE `.wrap_target` so they only run on initial enable (or explicit re-jump to entry_point); subsequent frames stay phase-locked by cycle-identity, not by per-frame re-sync. The earlier draft used a 32-iteration loop with `in pins, 1 [1]` which made each loop body 3 PIO cycles per bit instead of 2 — receiver sampled at 1.5× the master's bit rate, and the wait re-sync at every wrap caught LRCLK at random phases. *Last updated: 2026-05-03*

### Vendor Commands

| Code | Command | Direction | Description |
|------|---------|-----------|-------------|
| 0xCA | REQ_SET_I2S_DIN_PIN | IN* | Set I2S DIN pin (wValue=pin, returns status byte) |
| 0xCB | REQ_GET_I2S_DIN_PIN | IN | Get I2S DIN pin (returns uint8_t) |
| 0xCC | REQ_GET_I2S_INPUT_STATUS | IN | Get I2S input status (16-byte `I2sInputStatusPacket`) |

*0xCA uses the immediate-response SET pattern (same as `REQ_SET_SPDIF_RX_PIN`).

DIN pin change is rejected while I2S input is active. `is_pin_in_use()` rejects collisions with the DIN pin.

### Library Extensions (`pico_audio_i2s_multi`)

- `audio_i2s_clock_domain_configure(bck_pin, phantom_pio, phantom_sm)` — sets the BCK pin and phantom slot location. Called once at boot and after any BCK pin change.
- `audio_i2s_clock_domain_begin_transaction` / `_commit_transaction` — depth-counter transaction API for bulk changes; election runs once at outermost commit.
- `audio_i2s_clock_domain_clkout_only_start/stop/is_active/set_frequency` — internal helpers (called from the election; no longer called directly by `i2s_input.c`).
- `audio_i2s_clock_domain_bck_pin/lrclk_pin/sample_freq/is_active` (Phase 1) — read-only views; reflect TX-master state or phantom state, whichever is active.
- `audio_i2s_rx_setup/set_enabled/set_frequency/teardown/get_write_idx` — generic RX instance API. Slot count and slot width are stored on the instance for future TDM/multi-DIN support, but Phase 2 enforces stereo (slot_count=2, slot_width_bits=32).
- `audio_i2s_setup` (modified in Phase 2.5): `clock_master` flag in config is ignored; library elects internally based on registry order.
- `audio_i2s_update_all_frequencies` (extended in Phase 2.5): also updates phantom and RX dividers when present.

### Persistence

- I2S DIN pin: device-level setting in `PresetDirectory` v3 (`i2s_din_pin` field). Migration v2 → v3 defaults missing field to `PICO_I2S_DIN_PIN_DEFAULT` (GPIO 18). v1 → v3 migration also defaults it.
- `WireInputConfig.i2s_din_pin` (1 byte from existing reserved area; no `WIRE_FORMAT_VERSION` bump).
- Active input source — already covered by `WireInputConfig.input_source` and `PresetSlot.input_source` (V13). The I2S enum value (=2) fits the existing `uint8_t` field.

### Forward-compatibility Hooks

- The RX PIO program patches its `wait gpio` operands at runtime, so any BCK/LRCLK pin pair is supported.
- The library-level `audio_i2s_rx_instance_t` carries `slot_count` and `slot_width_bits` so future TDM (1 RX, slot_count=8) or multi-DIN (N RX, each slot_count=2) builds on the same API without library changes.
- The `audio_i2s_clock_mode_t` enum already includes `AUDIO_I2S_CLOCK_MODE_SLAVE`; future slave-mode work implements the slave-side rate measurement subsystem and decides on servo-vs-resampler for outputs, without touching the existing master code.
