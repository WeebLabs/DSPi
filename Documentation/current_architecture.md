...
- **Fill trim** (only when `fill_slot >= 0`), retuned 2026-07-22 after the first hardware capture (the divider-servo gains, expressed on the continuous VCXO, limit-cycled the integrator across its rails at ~10 ppm/s and unlocked external DAC receiver PLLs): a proportional term continuous beyond a ±2-buffer deadband (1.5 ppm per buffer beyond the edge, target 8 of 16 buffers) plus a clamped centering integrator gated on NONZERO fill error (0.005 ppm per tick per buffer, clamp ±10 ppm) so a centred fill cannot pump it. Fill responds to a rate trim through an integration and is quantized to whole buffers, so a micro limit cycle is structural; the clamp bounds its amplitude and a global slew limiter (0.5 ppm per ~20 ms tick, ~25 ppm/s, state zeroed by `input_servo_reset()`) bounds its slew below what downstream receiver PLLs track. Signs follow the ppm convention (overfull consumer = outputs too slow = positive ppm = speed sys_clk up).
