...
An optional master clock output for DACs that require an external clock reference. **MCK is generated by the hardware CLK_GPOUTn block — not by a PIO state machine.** clk_sys is divided by the CLK_GPOUTn 24.8 divider and routed directly to the chosen GPIO pad. No PIO SM is consumed; the CPU is uninvolved.

| Property | Value |
|----------|-------|
| Default GPIO | 21 (= clk_gpout0 on both RP2040 and RP2350) |
| Default multiplier | 128x Fs |
| Supported multipliers | 128x or 256x |
| Source | clk_gpout0 / 1 / 2 / 3 (chosen by GPIO; see below) |
| Hardware cost | One CLK_GPOUTn block (no PIO SM, no CPU) |

**Pin constraint:** GPIO must be a clk_gpout-capable pin. On both RP2040 and RP2350, `GPIO_TO_GPOUT_CLOCK_HANDLE(gpio)` returns the matching `clk_gpout0..3` instance:

| GPIO | Clock | Both platforms | RP2040 usable? | RP2350 usable? |
|------|-------|----------------|----------------|----------------|
| 13 | clk_gpout0 (RP2350 only) | RP2350 | — | ✅ |
| 15 | clk_gpout1 (RP2350 only) | RP2350 | — | ✅ |
| 21 | clk_gpout0 | both | ✅ (default) | ✅ (default) |
| 23 | clk_gpout1 | both | ❌ (SMPS PS pin) | ❌ |
| 24 | clk_gpout2 | both | ❌ (VBUS detect) | ❌ |
| 25 | clk_gpout3 | both | ❌ (on-board LED) | ❌ |

`REQ_SET_MCK_PIN` rejects pins that are not gpout-capable with `PIN_CONFIG_INVALID_PIN` (status `0x01`).

MCK frequency = multiplier × sample_rate. The MCK frequency updates automatically when the device sample rate changes (e.g., 44.1 kHz to 48 kHz). The CLK_GPOUTn block accepts glitchless divider changes — runtime updates via `audio_i2s_mck_update_frequency()` write the new divider directly to the clock generator without disabling MCK.

MCK jitter characteristics at sys_clk = 307.2 MHz (CLK_GPOUTn divider = sys_clk × 256 / (Fs × multiplier) in 24.8 fixed-point):

| Sample Rate | Multiplier | MCK Freq | GPOUT Divider | Jitter |
|-------------|-----------|----------|---------------|--------|
| 48 kHz | 128x | 6.144 MHz | **50.0** (integer) | **Zero** |
| 48 kHz | 256x | 12.288 MHz | **25.0** (integer) | **Zero** (was fractional 12.5 with PIO toggle) |
| 96 kHz | 128x | 12.288 MHz | **25.0** (integer) | **Zero** (was fractional 12.5 with PIO toggle) |
| 96 kHz | 256x | 24.576 MHz | 12.5 (fractional) | Fractional (sanitiser forces 128x at 96 kHz) |
| 44.1 kHz | 128x | 5.6448 MHz | 54.43… (fractional) | Fractional |
| 44.1 kHz | 256x | 11.2896 MHz | 27.21… (fractional) | Fractional |

**Recommendation:** Use 48 kHz × either multiplier for zero-jitter MCK on the GPOUT path — both 128x and 256x give integer dividers. Most modern DACs (ES9038, PCM5102, AK4490) accept 128x or 256x MCK. DACs that don't need MCK can leave it disabled entirely.

**Migration note:** Earlier firmware (≤ 2026-05-02) drove MCK from PIO1 SM1 with a 2-instruction toggle program. The PIO route doubled the effective divider (`sys_clk × 128 / target_MCK`) and gave fractional dividers for 48 kHz × 256× and 96 kHz × 128×. The CLK_GPOUTn route halves the divider math (`sys_clk × 256 / target_MCK`), produces integer dividers in those cases, and frees PIO1 SM1 for other uses. The default MCK pin moved from GPIO 13 to GPIO 21 in this migration; presets saved with the old default will be rejected by `REQ_SET_MCK_PIN` validation if reapplied (use bulk SET to overwrite, or pin via `REQ_SET_MCK_PIN` once in the new firmware).
...
| PIO1 SM1 | **Free** — was MCK PIO toggle, now driven by CLK_GPOUTn hardware block |
| PIO2 (RP2350) | S/PDIF input + ASRC |
| CLK_GPOUT0 | MCK (default GPIO 21; alternate GPIO 13 on RP2350) |
